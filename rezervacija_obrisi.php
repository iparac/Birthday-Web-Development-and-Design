...
if ($sqlRezultatDelete) {
     ...
     $sqlSelectTipDnevnikId = "SELECT naziv FROM tip_dnevnik WHERE tip_dnevnik_id = 12";
     ...
     $sqlDnevnikInsert = "INSERT into dnevnik (korisnik_id, tip_dnevnik_id, radnja, datum_vrijeme)
            VALUES ('$korisnik_id', 12, '$nazivTip', NOW())";
     $sqlRezultatDnevnikInsert = mysqli_query($conn, $sqlDnevnikInsert);

    header("Refresh:0; url=rezervacija_pregledaj.php");
} else {
    echo 'Nije obrisano';
}
  //  }
